...
4. Compute the coefficient of variation (CV) of each column in the `steam` dataset.
5. Create a toy data frame object with at least 5 rows and 3 columns.
...
greeting()
```
2. Write a for loop to sum up all the elements of a vector, compare the result of simply using the function `sum`. Use the function `Sys.time()` to record the time, for example:
```{r, eval=F}
start<- Sys.time()
...
4. Make an operator %$>€% to detect if the amount of money of the left hand side is greater the the right hand side. Where the left hand side is in usd (\$) and the right hand side is in euro (€). The exchange rate is 1€ = 1.1\$.
5. Use sapply to calcluate the object size of all the objects under current environment. Hint: you will need functions `ls()` and `object.size()`.